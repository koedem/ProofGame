...
uint64_t zobrist::zobrist_table[NPIECES + 1][NSQUARES];
...
uint64_t Position::zobrist_change_capture(Square from, Square to) {
    return zobrist::zobrist_table[board[from]][from] ^ zobrist::zobrist_table[board[from]][to]
           ^ zobrist::zobrist_table[board[to]][to];
}
...
uint64_t Position::zobrist_change_quiet(Square from, Square to) {
    return zobrist::zobrist_table[board[from]][from] ^ zobrist::zobrist_table[board[from]][to];
}
