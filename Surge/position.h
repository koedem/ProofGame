...
//gk		hash(0), pinned(0), checkers(0) {
    ...
    inline uint64_t zobrist_put_piece(Piece pc, Square s) {
        return zobrist::zobrist_table[pc][s];
    }
    ...
    inline uint64_t zobrist_remove_piece(Square s) {
        return zobrist::zobrist_table[board[s]][s];
    }

	void move_piece(Square from, Square to);
	void move_piece_quiet(Square from, Square to);
    uint64_t zobrist_change_quiet(Square from, Square to);
    uint64_t zobrist_change_capture(Square from, Square to);
    ...
    template<Color C> uint64_t zobrist_change_move(Move m);
};
...
// Calculate the hash change that a move would make
template<Color C>
uint64_t Position::zobrist_change_move(const Move m) {
    MoveFlags type = m.flags();

    switch (type) {
        case QUIET:
        case DOUBLE_PUSH:
            return zobrist_change_quiet(m.from(), m.to());
        case OO:
            if (C == WHITE) {
                return zobrist_change_quiet(e1, g1) ^ zobrist_change_quiet(h1, f1);
            } else {
                return zobrist_change_quiet(e8, g8) ^ zobrist_change_quiet(h8, f8);
            }
        case OOO:
            if (C == WHITE) {
                return zobrist_change_quiet(e1, c1) ^ zobrist_change_quiet(a1, d1);
            } else {
                return zobrist_change_quiet(e8, c8) ^ zobrist_change_quiet(a8, d8);
            }
        case EN_PASSANT:
            return zobrist_change_quiet(m.from(), m.to()) ^ zobrist_remove_piece(m.to() + relative_dir<C>(SOUTH));
        case PR_KNIGHT:
            return zobrist_remove_piece(m.from()) ^ zobrist_put_piece(make_piece(C, KNIGHT), m.to());
        case PR_BISHOP:
            return zobrist_remove_piece(m.from()) ^ zobrist_put_piece(make_piece(C, BISHOP), m.to());
        case PR_ROOK:
            return zobrist_remove_piece(m.from()) ^ zobrist_put_piece(make_piece(C, ROOK), m.to());
        case PR_QUEEN:
            return zobrist_remove_piece(m.from()) ^ zobrist_put_piece(make_piece(C, QUEEN), m.to());
        case PC_KNIGHT:
            return zobrist_remove_piece(m.from()) ^ zobrist_remove_piece(m.to())
                    ^ zobrist_put_piece(make_piece(C, KNIGHT), m.to());
        case PC_BISHOP:
            return zobrist_remove_piece(m.from()) ^ zobrist_remove_piece(m.to())
                    ^ zobrist_put_piece(make_piece(C, BISHOP), m.to());
        case PC_ROOK:
            return zobrist_remove_piece(m.from()) ^ zobrist_remove_piece(m.to())
                    ^ zobrist_put_piece(make_piece(C, ROOK), m.to());
        case PC_QUEEN:
            return zobrist_remove_piece(m.from()) ^ zobrist_remove_piece(m.to())
                    ^ zobrist_put_piece(make_piece(C, QUEEN), m.to());
        case CAPTURE:
            return zobrist_change_capture(m.from(), m.to());
    }
    return 0;
}
